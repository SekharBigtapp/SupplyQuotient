...
export class LoginComponent implements OnInit {
  ...
  clkSignin() {
    if (this.LoginForm.valid) {
      ...
      this.loginService.getLoginDetails(loginJsonData).subscribe((response) => {
  ...
  onSuccessfullLogin(response: any) {
    ...
    this.useridle.onTimerStart().subscribe((count) => {
      if (count > 1) {
        let obj = {
          "Username": this.loginData.userdetails.userId,
          "Token_generated": this.loginData.userdetails.userToken
          }
        this.loginService.UserLogout(obj).subscribe((resp: any) => {
          localStorage.clear();
          this.dataStorage.isUserLoggedIn = false;
          this.router.navigateByUrl('');
        });
